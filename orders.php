...
<body>
  ...
  <div class="container-fluid">
    ...
    <form action="orders.php" method="post" class="form-horizontal">
      ...
      <label for="pid" class="col-sm-3 control-label">Order ID</label>
      <div class="col-sm-9">
     ...
     <?php
        ...
        $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
          $stmt = $conn->prepare("SELECT * FROM tbl_staffs_a161017_pt2");
        $stmt->execute();
     ...
     <div class="col-sm-9">
      <select name="cid" class="form-control" id="cname" required>
      <?php
      try {
        $conn = new PDO("mysql:host=$servername;dbname=$dbname", $username, $password);
        $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
          $stmt = $conn->prepare("SELECT * FROM tbl_customers_a161017_pt2");
        $stmt->execute();
        $result = $stmt->fetchAll();
      }
      catch(PDOException $e){
    ...
    <div class="col-xs-12 col-sm-10 col-sm-offset-1 col-md-8 col-md-offset-2">
     ...
     // Read
      ...
      try {
        $conn = new PDO("mysql:host=$servername;dbname=$dbname", $username, $password);
        $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
        $sql = "SELECT * FROM tbl_orders_a161017, tbl_staffs_a161017_pt2, tbl_customers_a161017_pt2 WHERE ";
        $sql = $sql."tbl_orders_a161017.fld_staff_id = tbl_staffs_a161017_pt2.fld_staff_id and ";
        $sql = $sql."tbl_orders_a161017.fld_customer_id = tbl_customers_a161017_pt2.fld_customer_id";
    ...
    <div class="col-xs-12 col-sm-10 col-sm-offset-1 col-md-8 col-md-offset-2">
      <nav>
          ...
          try {
            $conn = new PDO("mysql:host=$servername;dbname=$dbname", $username, $password);
            $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $stmt = $conn->prepare("SELECT * FROM tbl_orders_a161017");
            $stmt->execute();
            $result = $stmt->fetchAll();
